...
```bash
# 0. Regenerate the themed installer artwork (only needed if the design changed)
node tools/gen-installer-images.js
...
"11" lettering, indicator lamps, toggle switches) generated by
`tools/gen-installer-images.js` into `src-tauri/installer/*.bmp`:
