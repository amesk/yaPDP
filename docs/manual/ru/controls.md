...
|---|---|---|
| {.disk .control-cell} ![Кнопка magic wand](assets/images/manual/btn-magicwand.png){.control-btn} Волшебная палочка{.control-name} | Правый верхний угол (все страницы, кроме Info) | Список быстрого запуска — выбирает гостевую ОС, перенастраивает, перезагружает и вводит загрузку/логин. См. Быстрый старт. |
