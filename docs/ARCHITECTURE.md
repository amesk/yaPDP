...
| [`assets/Model-33-ASR.svg`](../assets/Model-33-ASR.svg) | Model 33 ASR artwork — the console teletype cabinet used as the page backdrop. Its markers layer (`Keyboard`, `Apron`, `PuncherControl`, `PuncherTape`, `ReaderControl`, `ReaderTape`, `Paper`, `Caret`, each rect `display: none`) carries the rects the HTML controls are anchored to: the page reads them at load (`ttyMarkerVars` in `src/pdp11-app.js`) and writes them into the rig's `--tty-*` variables, so editing the artwork moves the controls (a marker rectangle carrying its own `rotate()`/`matrix()` counts as a four-point quad and is projected with a `matrix3d()`). Two layers are NOT part of the backdrop: `Foreground` is inlined (with `<defs>`, for its gradients) into `#tty-foreground`, which stacks above every overlay — that is how parts of the drawing cover the keys, the printed sheet and the tapes — while `ForegroundBack` goes into `#tty-foreground-back`, stacked BETWEEN the two hanging tapes (10 < 11 < 12), so the punched tape comes out OVER the punch tongue there while the reader tape still slips UNDER its own (layer lookup: `ttyArtLayerId`, Inkscape label first). `css/g60printer.css` keeps the same numbers as a fallback; `tests/teletype-svg-backdrop.test.js` pins parser, numbers, both layers and the never-paint rule |
