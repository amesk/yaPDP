 ...
 *      punch, output bytes punch rows, DC4 (0x14) disengages and punches
 *      nothing of its own; the control bytes are never printed themselves.
...
async function main() {
    ...
    try {
        ...
        // A fresh tape is blank: no lead-in is punched automatically, so the
        // row counts below are absolute (data rows start at row 0;
        // tapeRowTracks indexes from the BOTTOM of the tape).
        await setTtyMode(page, "local"); // keep the garbage off the line
        await punchOn(page);
        await tapeClear(page);
        await pressKey(page, { code: "A".charCodeAt(0) });
        await waitFor(async () => (await tapeRows(page)) === 1, 10000);
        await pressKey(page, { code: "B".charCodeAt(0) });
        await waitFor(async () => (await tapeRows(page)) === 2, 10000);
        await page.evaluate(() => {
        ...
        await waitFor(async () => (await tapeRows(page)) === 3, 10000);
        const rowA = await tapeRowTracks(page, 0);
        const rowBC = await tapeRowTracks(page, 1);
        const rowDel = await tapeRowTracks(page, 2);
        const A = 0x41, BC = 0x41 | 0x43, DEL = 0x7F;
        check("BSP row exists after A, B, BSP, C, RUBOUT (3 data rows)",
        ...
        await setTtyMode(page, "line");
        await tapeClear(page);
        const rowsBefore6 = await tapeRows(page); // blank tape: nothing punched yet
        ...
        const rowsBeforeDc4 = await tapeRows(page);
        await page.evaluate(() => window.g60ConsoleWrite(0x14)); // DC4
        // DC4 stops the punch and punches nothing of its own: on the real
        // machine the operator punched any trailing NULs by hand before
        // switching off. The row count must be unchanged by the disengage.
        const rowsAfterDc4 = await tapeRows(page);
        check("DC4 punches nothing on the way out",
            rowsAfterDc4 === rowsBeforeDc4,
            "rows: " + rowsBeforeDc4 + " -> DC4 -> " + rowsAfterDc4);
        ...
        // The tape holds exactly the 4 data rows: no leader is punched.
        await waitFor(async () =>
            (await tapeRows(page)) === 4, 10000);
        ...
        // Byte-exact: "SAVE" and nothing else — the download is the punched
        // rows themselves, so a tape used for host interchange round-trips
        // byte for byte with no mechanical field added at either end.
        const dataOk = bytes.length === 4 &&
            bytes[0] === 0x53 && bytes[1] === 0x41 &&
            bytes[2] === 0x56 && bytes[3] === 0x45;
        check("Save tape downloads a byte-exact .ptap (the data rows alone)",
            dataOk,
