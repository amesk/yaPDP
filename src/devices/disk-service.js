...
(function (global) {
    ...
    class DiskService {
        ...
        async io(controlBlock, operation, position, address, count, options) {
            if (typeof process !== "undefined" && process.env && process.env.DEBUG_DISK) console.log("DISK.io op=" + operation + " pos=" + position + " addr=" + address + " count=" + count + " url=" + controlBlock.url);
            const drive = this._driveFor(controlBlock);
